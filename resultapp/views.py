...
def face_register(request):
    ...
    if request.method == 'POST':
        ...
        if not FACE_RECOGNITION_AVAILABLE:
            messages.error(
                request,
                "Face Recognition library is not properly installed. "
                "Please ensure dlib-bin, face-recognition, and setuptools<70 are installed."
            )
        ...
        try:
            ...
            if not encodings:
                ...
                messages.warning(
                    request,
                    f"No face detected for {student.name}. "
            ...
            messages.success(
                ...
                "Don't forget to 'Train All Faces' to finalize."
            )
...
def face_train(request):
    ...
    if not FACE_RECOGNITION_AVAILABLE:
        return JsonResponse({
            'status': 'error',
            'message': 'Face Recognition library not installed. See requirements.'
        }, status=500)
    ...
    try:
        ...
        for student in Student.objects.filter(status=1).exclude(photo=''):
            ...
            try:
                ...
                if not os.path.exists(img_path):
                    continue
                # Reuse individual encoding if exists to speed up training
                enc_path = os.path.join(enc_dir, f'{student.id}.pkl')
        ...
        return JsonResponse({
            'status': 'success',
            'message': f'Training complete. {len(known_ids)} student(s) optimized.'
        })

    except Exception as e:
        return JsonResponse({'status': 'error', 'message': f"Training failed: {str(e)}"}, status=500)


def _generate_frames():
    """Generator that yields MJPEG frames from webcam with face identification."""
    if not FACE_RECOGNITION_AVAILABLE:
        return
